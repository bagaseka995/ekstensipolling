...
// ─── Bot Logic (Konteks C: Halaman Form) ───────────────────────────────────────
  ...
  const intervalId = setInterval(async () => {
    ...
    //
    // TAHAP 5 - Halaman PLN (card dengan radio button)
    //
    // Deteksi halaman PLN: ada teks "pln" DAN ada radio/card selection
    const hasPLNText = bodyText.toLowerCase().includes('pln');
    const hasEnergyCompany = bodyText.toLowerCase().includes('perusahaan penyedia energi') ||
      bodyText.toLowerCase().includes('energi & telekomunikasi: energy') ||
      bodyText.toLowerCase().includes('klik logo perusahaan');

    if (hasPLNText && hasEnergyCompany && !sedangMenungguLogin) {
      isActionRunning = true;
      try {
        notifyPopup(`? TAHAP 5: Memilih PLN.`, 'status-running', currentIndex, totalEmails);
        await sleep(800);

        // Strategi 1: Cari elemen terkecil yang TEPAT mengandung "PLN"
        const allEls = [...document.querySelectorAll('div, a, button, label, span, li, article, section')];
        const plnEl = allEls.find(el => {
          const rect = el.getBoundingClientRect();
          if (rect.width === 0 || rect.height === 0) return false;
          const txt = el.textContent.replace(/\s+/g, ' ').trim();
          // Harus mengandung "PLN" tapi ukurannya kecil (bukan container utama)
          return txt.includes('PLN') && txt.length < 120 && rect.height < 200;
        });

        console.log(`[BOT-T5] plnEl found: ${!!plnEl}, tag: ${plnEl?.tagName}, text: ${plnEl?.textContent?.trim()?.slice(0,60)}`);

        if (plnEl) {
          // Klik card PLN
          plnEl.scrollIntoView({ block: 'center' });
          plnEl.click();
          await sleep(500);

          // Juga coba klik radio/input di dalam card
          const radio = plnEl.querySelector('input[type="radio"]') || plnEl.querySelector('[role="radio"]');
          if (radio) {
            radio.click();
            await sleep(300);
            console.log(`[BOT-T5] Radio inside PLN clicked`);
          }

          // Coba juga dispatch event pada parent card
          let cardParent = plnEl;
          for (let i = 0; i < 5; i++) {
    ...
    // TAHAP 3 - Pilih "Energy" (sub-sektor) - deteksi via URL
    //
    ...
    if (isSubSektor && !sedangMenungguLogin) {
      isActionRunning = true;
      try {
      notifyPopup(`? TAHAP 3: Memilih Energy.`, 'status-running', currentIndex, totalEmails);
      await sleep(800);

      // Cari card "Energy" - elemen visible yang mengandung "energy" tapi bukan container besar
      const allCandidates = [...document.querySelectorAll('a, button, div, li, article, h1, h2, h3, p, span')];
      const energyEl = allCandidates.find(el => {
        const rect = el.getBoundingClientRect();
        if (rect.width === 0 || rect.height === 0) return false;
        const txt = el.textContent.replace(/\s+/g, ' ').trim().toLowerCase();
        // Harus mengandung "energy" tapi TIDAK "telecommunication" (hindari container besar)
        return txt.includes('energy') && !txt.includes('telecommunication') && txt.length < 150;
      });

      if (energyEl) {
        energyEl.scrollIntoView({ block: 'center' });
        ['mousedown', 'mouseup', 'click'].forEach(type =>
          energyEl.dispatchEvent(new MouseEvent(type, { bubbles: true, cancelable: true, view: window }))
        );
      } else {
        console.warn('[BOT] Elemen Energy tidak ditemukan di halaman sub-sektor!');
      }
      } finally {
        await sleep(2500);
        isActionRunning = false;
